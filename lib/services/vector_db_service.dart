...
// );
typedef _Sqlite3ExecNative = Int32 Function(SQLiteDBPointer pDb, Pointer<Utf8> sql, Pointer<Void> callback, Pointer<Void> pArg, Pointer<Pointer<Utf8>> pzErrMsg);
typedef _Sqlite3ExecDart = int Function(SQLiteDBPointer pDb, Pointer<Utf8> sql, Pointer<Void> callback, Pointer<Void> pArg, Pointer<Pointer<Utf8>> pzErrMsg);
...
class VectorDbService {
  ...
  Future<void> initialize({String dbName = 'vector_database.db'}) async {
    ...
    try {
      ...
      _sqlite3Exec = _nativeLib!.lookupFunction<_Sqlite3ExecNative, _Sqlite3ExecDart>('sqlite3_exec');
      _sqlite3Free = _nativeLib!.lookupFunction<_Sqlite3FreeNative, _Sqlite3FreeDart>('sqlite3_free');
  ...
  Future<void> addEmbedding({
    ...
    _log.info('VectorDbService: addEmbedding called for id: $id (Insertion FFI not yet implemented)...');
  ...
  Future<void> _createTables() async {
    ...
    const sqlCreateEmbeddingsTable = 'CREATE VIRTUAL TABLE IF NOT EXISTS embeddings USING sqlitevec(embedding FLOAT[384]);';
  ...
  Future<void> _executeSql(String sql, String tableNameForLogging) async {
    ...
    try {
      ...
      eventLogger('VectorDB: Executing CREATE for $tableNameForLogging table...');
      final execResult = _sqlite3Exec(_db, sqlC, nullptr, nullptr, errMsgPointer);

      if (execResult != SQLITE_OK) {
        final errorMessage = errMsgPointer.value.toDartString();
        eventLogger('VectorDB: FAILED to create $tableNameForLogging table. Code: $execResult, Error: $errorMessage');
        _sqlite3Free(errMsgPointer.value.cast<Void>());
        throw Exception('Failed to create $tableNameForLogging table: $errorMessage');
      }
